...
class WriteOffRepositoryMixin {
  List<String> getItems(GroupBy groupBy, List<Map<String, dynamic>> data) {
    ...
    if (groupBy == GroupBy.day) {
      return data.map((e) {
        final date = e['StockWriteoffDetail.date.day'] as String;
    ...
    if (groupBy == GroupBy.month) {
      return data.map((e) {
        final date = e['StockWriteoffDetail.date.month'] as String;
        return FormatUtils.formatWithCustomFormatter(
    ...
    if (groupBy == GroupBy.quarter) {
      return data.map((e) {
        final date = e['StockWriteoffDetail.date.quarter'] as String;
        return FormatUtils.formatToQuarters(date);
      }).toList();
    }
    if (groupBy == GroupBy.year) {
      return data.map((e) {
        final date = e['StockWriteoffDetail.date.year'] as String;
        return FormatUtils.formatWithCustomFormatter(date, DateFormat('yyyy'));
      }).toList();
    }
    if (groupBy == GroupBy.type) {
      return data
          .map((e) => (e['StockWriteoffDocument.type']) as String)
          .toList();
    }
    return [];
  }
  ...
  Annotation getDimension(GroupBy groupBy, Map<String, dynamic> dimensionMap) {
    ...
    switch (groupBy) {
      ...
      case GroupBy.product:
        ...
        break;
      case GroupBy.day:
        dimensionKey = 'StockWriteoffDetail.date.day';
        break;
      case GroupBy.month:
        dimensionKey = 'StockWriteoffDetail.date.month';
        break;
      case GroupBy.quarter:
        dimensionKey = 'StockWriteoffDetail.date.quarter';
        break;
      case GroupBy.type:
        dimensionKey = 'StockWriteoffDocument.type';
        break;
      case GroupBy.year:
        dimensionKey = 'StockWriteoffDetail.date.year';
        break;
      default:
